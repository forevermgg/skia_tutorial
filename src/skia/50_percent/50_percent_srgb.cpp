...
void draw(SkCanvas *canvas) {
  ...
  // 假设 make() 函数返回一个 SkShader 对象（例如前面提到的 make_bw_dither()
  // 函数）。
  p.setShader(make());

  // 使用 SkPaint 对象（包含 SkShader
  // 对象）在画布上绘制一个矩形，左上角位于原点，宽度和高度都是 50。
  ...
  // 使用之前设置的 SkPaint 对象（包含 SkShader
...
int main(int argc, char *const argv[]) {
  ...
  SkFILEWStream save_file(
      (current_directory + PATH_SEPARATOR + "50_percent_srgb.png")
          .c_str()); // 创建文件输出流
