...
static sk_sp<SkShader> make_bw_dither() {
    // 创建一个宽度为 2，高度为 2 的新 SkSurface 对象。这将创建一个用于绘制的位图画布。
    auto surf = SkSurfaces::Raster(SkImageInfo::MakeN32Premul(2, 2));
    // 将画布的颜色设置为白色。这将清除画布上的任何内容，并将其填充为白色。
    surf->getCanvas()->drawColor(SK_ColorWHITE);
    // 使用默认的 SkPaint 对象（默认为黑色），绘制一个矩形。矩形的左上角位于原点，宽度和高度都是 1。
    surf->getCanvas()->drawRect({0, 0, 1, 1}, SkPaint());

    // 使用默认的 SkPaint 对象（默认为黑色），绘制另一个矩形。矩形的左上角位于 (1, 1)，宽度和高度都是 1。
    surf->getCanvas()->drawRect({1, 1, 2, 2}, SkPaint());

    // 从 SkSurface 对象创建一个 SkImage 快照，然后从 SkImage 创建一个 SkShader 对象。
    // SkShader 的水平和垂直平铺模式都设置为 kRepeat，采样选项设置为 kLinear。
...
void draw(SkCanvas* canvas) {
    ...
    // BW Dither
    // 在画布上移动原点（0,0）。在这个例子中，我们将原点向右移动 5 单位，向下移动 5 单位。
    canvas->translate(5, 5);
    // 创建一个新的 SkPaint 对象，用于绘制形状。这个对象的着色器被设置为一个黑白抖动模式。
    SkPaint p;
    p.setShader(make_bw_dither());
    // 使用之前创建的 SkPaint 对象，绘制一个矩形。矩形的左上角位于原点，宽度和高度都是 100。
    canvas->drawRect({0, 0, 100, 100}, p);
    // 创建一个新的 SkPaint 对象，用于绘制黑色文本。
    SkPaint black;
    // 使用之前创建的 SkFont 对象和 SkPaint 对象，绘制一行文本。文本的左下角位于原点，向右移动 0 单位，向下移动 125 单位。
    canvas->drawString("BW Dither", 0, 125, font, black);

    // Scaled BW Dither
    // 在画布上移动原点。在这个例子中，我们将原点向右移动 105 单位，向下移动 0 单位。
    canvas->translate(105, 0);
    // 保存当前画布的状态。这将允许我们在稍后恢复画布的状态。
    canvas->save();
    // 缩放画布。在这个例子中，我们将画布的宽度和高度都缩小一半。
    canvas->scale(0.5, 0.5);
    // 使用之前创建的 SkPaint 对象，绘制一个矩形。矩形的左上角位于原点，宽度和高度都是 200。
    ...
    // 使用之前设置的不透明灰色 SkPaint 对象，绘制一个矩形。矩形的左上角位于原点，宽度和高度都是 100。
    canvas->drawRect({0, 0, 100, 100}, p);
    // 在新位置绘制一行文本。
...
int main(int argc, char *const argv[]) {
    ...
    SkFILEWStream save_file((current_directory + PATH_SEPARATOR + "50_percent_gray.png").c_str()); //创建文件输出流
