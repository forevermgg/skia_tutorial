...
void canvas_drawShadow(SkCanvas *canvas, SkPath *path, SkScalar elevation,
                       SkScalar devicePixelRatio, SkColor color,
...
void DrawShadow(SkCanvas *canvas, const SkPath &path, SkColor color,
...
int main() {
  ...
  SkFILEWStream file(
      (current_directory + PATH_SEPARATOR + "drawable_circle_rect_shadow.png")
          .c_str()); // 创建文件输出流
  if (!file.isValid()) {
    return 1;
  }
