 ...
 * @param path SkPath 对象
 */
void DebugShowSkiaDrawPath(const SkCanvas *canvas, const SkPaint &paint,
                           const SkPath &path);
 ...
 */
void DebugShowSkiaDrawRect(const SkCanvas *canvas, const SkPaint &paint,
                           const SkRect &rect);
...
bool ConvertBitmap(const SkBitmap &src_bitmap, SkBitmap *target_bitmap,
                   SkColorType target_ct, SkAlphaType target_at);
...
// Encodes the bitmap into a data:/image/png;base64,... url suitable to view in
// a browser after printing to a log. If false is returned, dst holds an error
// message instead of a URI.
