...
std::string drawSingleWaterMark(SkCanvas *canvas) {
  ...
  // 计算文本的宽度
  const char *text = "centforever 五一快乐 187********\u00a02024-04-14";
...
std::string drawWaterMarkModelOne(SkCanvas *canvas) {
  ...
  for (int i = 0; i < column + 1; i++) {
    for (int j = 0; j < row + 1; j++) {
      ...
      canvas->drawSimpleText(text, length, SkTextEncoding::kUTF8, textWidth / 2,
                             baseLineY + tileH / 2, font, paint);
      canvas->restore();
    }
  }
...
std::string drawWaterMarkModelTwo(SkCanvas *canvas) {
  ...
  for (int i = 0; i < column + 1; i++) {
    for (int j = 0; j < row + 1; j++) {
      ...
      startX = startW + i * tileW;
      startY = startH + j * tileH;
      canvas->drawSimpleText(text, length, SkTextEncoding::kUTF8, textWidth,
                             baseLineY, font, paint);
      canvas->restore();
    }
  }
...
std::string drawMultilinesWaterMark(SkCanvas *canvas) {
  ...
  font.getMetrics(&fontMetrics);
  float baseLineY = std::abs(fontMetrics.fDescent + fontMetrics.fAscent) / 2;
  // 文本高度：文本的高度就是实际绘制区域的高度，可以用(fontMetrics.descent -
  // fontMetrics.ascent)获取，因为ascent为负数，所以最终算出来的是两者的和
  ...
  for (int i = 0; i < setSizes; i++) {
      double gap = i / 2.0;
      padding = i > splitPoint ? (textPadding * (i - splitPoint)) : (textPadding
  * (-splitPoint + i)); std::cout << " 除数: " << gap << " 剩数: " << i % 2 <<
      " 分割点: " << splitPoint <<
      " padding: " << padding <<
      std::endl;
  }*/
    ...
    for (int j = 0; j < row + 1; j++) {
      ...
      for (int k = 0; k < subLabels.size(); k++) {
        ...
        canvas->drawSimpleText(
            text.c_str(), length, SkTextEncoding::kUTF8,
            SkScalar((percentWH / 2.0 - textWidth / 2.0) + percentWH / 4.0),
            SkScalar(fontHeight * k * 1.0 - fontHeight) + padding, font, paint);
    ...
    for (int j = 0; j < row + 1; j++) {
      ...
      for (int k = 0; k < subLabels.size(); k++) {
        ...
        if (k < basePosition) {
          ...
          baselineY =
              textTotalHeight / 2 - textHeight * (textLines - k - 1) - bottom;
        ...
        canvas->drawSimpleText(text.c_str(), length, SkTextEncoding::kUTF8,
                               -textWidth / 2, baselineY, font, paint);
      }
      canvas->restore();
    }
  }
