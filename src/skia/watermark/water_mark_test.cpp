...
bool loadImageToBitmap(std::string imagePath, SkBitmap *bitmap) {
  ...
  if (!data) {
    std::cerr << "Failed to create SkData from file: " << imagePath
              << std::endl;
    return false;
  }
  ...
  std::unique_ptr<SkCodec> codec =
      SkPngDecoder::Decode(SkMemoryStream::Make(data), &result);
  if (result != SkCodec::Result::kSuccess) {
    std::string errorMsg =
        SkStringPrintf("Could not create codec for \"%s\": %s.",
                       imagePath.c_str(), SkCodec::ResultToString(result))
            .c_str();
...
void DrawBackgroundBitmap(const SkBitmap &bitmap, SkCanvas *canvas,
                          const SkRect *prcDst) {
  SkSamplingOptions sampling;
  sk_sp<SkShader> bg_shader = bitmap.makeShader(
      SkTileMode::kRepeat, SkTileMode::kRepeat, sampling, nullptr);
  SkPaint bg_paint;
  bg_paint.setShader(bg_shader);
  canvas->drawRect(*prcDst, bg_paint);
}

void DrawBackgroundBitmap(const SkBitmap &bitmap, SkCanvas *canvas, float l,
