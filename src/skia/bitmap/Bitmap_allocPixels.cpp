...
void draw(SkCanvas *canvas) {
  // 创建一个空的 SkBitmap 对象
  SkBitmap bitmap;
  // 创建一个 SkImageInfo 对象，设置图像的宽度为 256，高度为
  // 64，颜色类型为灰度，alpha 类型为不透明
  SkImageInfo info =
      SkImageInfo::Make(256, 64, kGray_8_SkColorType, kOpaque_SkAlphaType);
...
int main(int argc, char *const argv[]) {
  ...
  SkFILEWStream save_file(
      (current_directory + PATH_SEPARATOR + "Bitmap_allocPixels.png")
          .c_str()); // 创建文件输出流
