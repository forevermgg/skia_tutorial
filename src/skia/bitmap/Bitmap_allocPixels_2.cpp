...
void draw(SkCanvas *canvas) {
  ...
  // 为 SkBitmap 分配像素内存，设置图像的宽度为 64，高度为
  // 64，颜色类型为灰度，alpha 类型为不透明
  bitmap.allocPixels(
      SkImageInfo::Make(64, 64, kGray_8_SkColorType, kOpaque_SkAlphaType));
  ...
  for (int y : {0, 64, 128, 192}) {
    // 在 offscreen 上绘制 source 位图，位图的 x 和 y 坐标都被向左和向上移动 y
    // 个单位
...
int main(int argc, char *const argv[]) {
  ...
  SkFILEWStream save_file(
      (current_directory + PATH_SEPARATOR + "Bitmap_allocPixels_2.png")
          .c_str()); // 创建文件输出流
