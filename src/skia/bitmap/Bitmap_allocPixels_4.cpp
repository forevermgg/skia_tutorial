...
public:
  bool allocPixelRef(SkBitmap *bitmap) override {
    ...
    sk_sp<SkPixelRef> pr = sk_sp<SkPixelRef>(new SkPixelRef(
        info.width(), info.height(), storage, info.minRowBytes()));
...
int main(int argc, char *const argv[]) {
  ...
  SkFILEWStream save_file(
      (current_directory + PATH_SEPARATOR + "Bitmap_allocPixels_4.png")
          .c_str()); // 创建文件输出流
