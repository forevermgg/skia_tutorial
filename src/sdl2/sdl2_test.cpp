 ...
 * https://nawwid.medium.com/how-to-create-your-first-sdl2-project-using-clion-ide-in-macos-9155764d5765
 */
#include <SDL.h>
#include <SDL_image.h>
#include <SDL_mixer.h>
 ...
 *
 * This uses SDL_CreateWindowAndRenderer. They can alternatively be created
 * separately. See SDL2 Docs
 */
...
void setup_header_text() {
  // See CMakeLists.txt to see how the resources folder is copied from the root
  // to the bin folder
  _font = TTF_OpenFont("resources/OpenSans-Regular.ttf", 18);

  SDL_Surface *textSurface = TTF_RenderText_Blended(
      _font, "Mouse Click and Drag Rect. Press [Escape] to Exit", white);
  ...
  // The value will probably be misreported as never used in CLion, however I
  // think it should always be set
  textSurface = NULL;
}
...
/**
 * Handles dragging the sample rectangle around. Demonstrates mouse motion
 * events.
 * @param e
 */
...
/**
 * The main game loop. Continues to loop until Escape or an SDL_Quit event
 * occurs
 */
void main_loop() {
  ...
  while (loop) {
    ...
    // Render the sample texture. We could use a source and/or destination rect
    // to render to but for now we'll just use it as a background
...
int main() {
  ...
  // Important note. These setup calls require the resources folder to be copied
  // to the bin folder. This is done via the CMakeList.txt file - see the 'file'
  // call towards the end. This means any changes to the resources after the
  // initial project load require the CMake project to be reloaded - in CLion
  // use File -> Reload CMake Project
